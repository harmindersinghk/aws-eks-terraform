...
module "aws_coredns" {
  ...
  helm_config = merge(
    ...
    var.self_managed_coredns_helm_config,
    {
      ...
      image_registry = local.amazon_container_image_registry_uris[data.aws_region.current.id]
    }
  )
...
module "aws_load_balancer_controller" {
  ...
  manage_via_gitops = var.argocd_manage_add_ons
  addon_context     = merge(local.addon_context, { default_repository = local.amazon_container_image_registry_uris[data.aws_region.current.id] })
}
